...
//$cfg_sw = query("/captival/auth:5/state");
//$cfg_master = query("/captival/auth:6/state");
...
//erial add for 802.1x and local radius server
...
function on_change_mode(s)
{
	...
	{
		f.captival_profile.value = "<?=$cfg_captival_profile?>";
...
function on_change_authentication(s)
{
    ...
    else if(s.value == 9)// 802.1x  //erial add for 802.1x and local radius server
	{
	...
	else
	{
		...
		if(f_final.f_channel_width_type.value==1)	
			{
				f.channel_width_ac.disabled=false;	
			}
			else
			{
   ...
   on_change_size();
	if(mode_value.value ==1 || mode_value.value == 2)
	{
		...
		if(f_final.f_channel_width_type.value==1)	
			{
				f.channel_width_ac.value=2;
				f.channel_width_ac.disabled=true;
			}
		else
		{
			f.channel_width.value=1;
...
function on_change_radius_type()   //erial add for 802.1x and local radius server
{
	var f = get_obj("frm");
...
function on_click_encryption()
{
	...
	if(f.encrypt[0].checked)
	{
		...
		f.confirm_key.disabled = true;
			if("<?=$cfg_wlmode?>" != 2)
			{
					if(f_final.f_channel_width_type.value==0)							
								f.channel_width.disabled=false;	
					else
								f.channel_width_ac.disabled=false;	
			}
		...
		else
		{
			...
			{
									f.channel_width_ac.value=0;
								f.channel_width_ac.disabled=true;	
			}
		}	
	}	
	else if(w_auth.value == 9)
	{
		if(f_final.f_channel_width_type.value==0)
			{						
								select_index(f.channel_width, 0);
								f.channel_width.disabled=true;	
			}
			else
			{
									select_index(f.channel_width_ac, 0);
								f.channel_width_ac.disabled=true;	
			}
	}
	if(mode_value.value==1 || mode_value.value == 2)
	{
		if(f_final.f_channel_width_type.value==0)
			...
			{
									select_index(f.channel_width_ac,2);
								f.channel_width_ac.disabled=true;	
			}
...
for("/runtime/wlan/inf:1/wdslistinfo/wdsscan")
{
	if(query("wdsmode")!="")
	{
	$wds_num++;	
...
function on_change_channel()
{
	...
	else
	{
		...
		if(f.channel_width.value == 2)
		{
			f.channel_width.value = 0;
		}
		f_final.f_channel_width_type.value=0;
	}
	on_click_encryption();
}
...
function init()
{
	...
	if(("<?=$cfg_band?>"== 1 && "<?=$cfg_wlan_domain?>" == 826)||("<?=$cfg_band?>"== 1 && "<?=$cfg_wlan_domain?>" == 818)||("<?=$cfg_band?>" != 0 && "<?=$cfg_wlan_domain?>" == 392))
	{
		get_obj("mode").style.display = "none";
	...
	if("<?=$cfg_band?>"== 1 && "<?=$cfg_wlmode?>" == 4) // 11g
	{
		get_obj("channel_width_ac").style.display = "";
		get_obj("channel_width").style.display = "none";
		select_index(f.channel_width_ac, "<?=$cfg_channel_width?>");
		f.channel_width_ac.disabled = false;	
		f_final.f_channel_width_type.value=1;
	}
	else
...
function on_change_cipher()
{
	var f = get_obj("frm");
	var f_final = get_obj("final_form");
	if("<?=$cfg_wlmode?>" != 2 && mode_value.value != 1 && mode_value.value != 2)
	{
		if(f_final.f_channel_width_type.value==0)							
								f.channel_width.disabled=false;	
					else
	...
	if((w_auth.value == 3 || w_auth.value == 2) && f.cipher.value == 2)
	{
		if(f_final.f_channel_width_type.value==0)		
		{					
								f.channel_width.value = 0;
...
function check()
{
	...
	if(f_final.f_channel_width_type.value == 1)
		f_final.f_channel_width.value = f.channel_width_ac.value;
	else
	...
	else if(f_final.f_auth.value == 2 || f_final.f_auth.value == 9) //eap or 802.1x //erial add for 802.1x and local radius server
	{
		...
		for(var l=1; l<=radius_srv_number; l++) // two server
		{
			...
			{
				if(!is_valid_ip3(get_obj(srv_obj+"_srv_"+l).value,0))
				{
		...
		<? if(query("/runtime/web/display/local_radius_server") =="1")	
			{echo "}";}
		?>	   	
		if(f.radius_type != null)   //erial add for 802.1x and local radius server
		{
			if(f.radius_type[0].checked == true)
			{
		...
		f_final.f_gkui.value = f.gkui.value;
		f_final.f_kui.value = f.kui.value;  //erial add for 802.1x and local radius server
...
<input type="hidden" name="f_channel_width_type"		value="">
...
<? if(query("/runtime/web/display/captival") != "1") {echo "<!--";} ?>
				<tr>
					<td id="td_left">
						<?=$m_captival_profile?>
					</td>
...
//if($cfg_sw == 1){echo "<option value=\"5\">".$m_sw."</option>";}
//if($cfg_master == 1){echo "<option value=\"6\">".$m_master."</option>";}
?>
						</select>
					</td>
				</tr>
...
for("/runtime/macaddr_ethernet")
{
	echo "<input type=\"hidden\" id=\"scan_mac_clone".$@."\" name=\"scan_mac_clone".$@."\" value=\"".query("addr")."\">\n";
		...
		echo "<td width=\"40\" align='center'><input type=\"radio\" name=\"scan_table\" id=\"scan_mac_table".$@."\" onclick=\"on_click_scan_mac_table(".$@.")\"></td>\n";	
		echo "<td width=\"150\">".query("addr")."</td>\n";
			echo "</tr>\n";
