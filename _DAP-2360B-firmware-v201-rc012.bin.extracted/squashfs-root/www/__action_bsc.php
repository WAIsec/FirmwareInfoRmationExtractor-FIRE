	...
	if($f_mode != 0)
	{
		...
		if($f_mode == 1) //apc
		{
			if($TITLE == "DAP-2695")
			{
				...
				else {set("wlmode",	4);} // 11a, mode = mixed n,a			
			}
			else
			{
				set("wlmode",1);
...
//					set("/wlan/inf:1/wpa/embradius/enable",0);
				}
		}
...
/*		set("/wlan/inf:1/aparray_enable", 0);
		...
		if(query("/wlan/inf:1/authentication") == 2 || query("/wlan/inf:1/authentication") == 4 || query("/wlan/inf:1/authentication") == 6 || query("/wlan/inf:1/authentication") == 9) 
		{$cfg_eap = 1;}
		if(query("/wlan/inf:1/wpa/embradius/state") == 1 && $cfg_eap == 1)
		{
			set("/wlan/inf:1/authentication", 0);
			set("/wlan/inf:1/wpa/wepmode", 0);
		}
		for("/wlan/inf:1/multi/index")
		{
			if(query("auth") == 2 || query("auth") == 4 || query("auth") == 6 || query("auth") == 9)
			{$cfg_multi_eap = 1;}
			if(query("embradius_state") == 1 && $cfg_multi_eap == 1)
			{	
				set("auth", 0);
				set("cipher", 0);
			}
...
/*
	if($ipv6_dirty > 0)
	{
		...
		set("/runtime/web/submit/trafficmgr", 1);
		set("/runtime/web/submit/wlan", 1);
	}
*/
	set("/runtime/web/next_page",$ACTION_POST);
