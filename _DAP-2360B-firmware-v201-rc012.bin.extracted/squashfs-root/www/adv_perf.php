...
function on_change_wlmode(s)
{
	...
	if(s.value != 2) // NOT Mixed g_b, a only
	{
		...
		if("<?=$cfg_wepmode?>"=="1" || "<?=$cfg_auth?>"=="9" || "<?=$cfg_wepmode?>"=="2")
		...
		else if(s.value == 1)
		{
			...
			{		//modified by yuejun for wmm
					...
					w_rate.options[0].text = "<?=$m_best_ac?>";
			}
	}
	else
	{
...
function init()
{
    ...
    if("<?=$cfg_ap_mode?>"==1 )
    {
    	f.g_mcast_bgn.disabled=true;
    	f.g_mcast_bg.disabled=true;
			f.a_mcast_an.disabled=true;
			f.a_mcast_a.disabled=true;
			f.mcast_n.disabled=true;
    }
    else
    {
    		f.g_mcast_bgn.disabled=false;
    	f.g_mcast_bg.disabled=false;
			f.a_mcast_an.disabled=false;
			f.a_mcast_a.disabled=false;
			f.mcast_n.disabled=false;
    }
	}
