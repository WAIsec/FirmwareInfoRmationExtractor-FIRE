...
$cfg_pop3_ip = get("h","/captival/pop3/server_ip");
...
function on_change_auto_copy()
{
	...
	if(f.auto_copy.checked)
	{
		...
		f.identity.value = "cn="+f.ldap_name.value+","+f.base_dn.value;
	}
	else
	{
...
function strchk_charactors(str)
{
...
function check_radius()
{
	...
	if(strchk_charactors(f.radius_sec.value) == false)
	{
		alert("<?=$a_invalid_sec?>");
		f.radius_sec.select();
		return false;
	...
	if(f.acc_mode.value == 1)
	{
		...
		if(strchk_charactors(f.acc_sec.value) == false)
		{
			alert("<?=$a_invalid_sec?>");
			f.acc_sec.select();
			return false;
...
function check_ldap()
{
	...
	if(strchk_charactors(f.ldap_name.value) == false)
	{
		alert("<?=$a_invalid_name?>");
		f.ldap_name.select();
		return false;
	...
	if(strchk_charactors(f.ldap_password.value) == false)
	{
		alert("<?=$a_invalid_sec?>");
			...
			<td colspan="2">
				...
				<table width="100%" border="0" align="center" <?=$G_TABLE_ATTR_CELL_ZERO?>>
					<tr height="25px">
						<td><?=$m_remote_type?></td>
						<td>
							<select id="remote_type" name="remote_type" disabled>
								<option value="0" selected><?=$m_spap?>
								<option value="1"><?=$m_peap?>
							</select>
						</td>
					</tr>
					<tr>
			...
			<td colspan="2">
				...
				<table width="100%" border="0" align="center" <?=$G_TABLE_ATTR_CELL_ZERO?>>
					...
					<tr>
						...
						<td><input type="text" maxlength="64" size="30" id="identity" name="identity" class="text" value="<?=$cfg_identity?>">
		...
		<tr>
			<td colspan="2">
				...
				<table width="100%" border="0" align="center" <?=$G_TABLE_ATTR_CELL_ZERO?>>
					<tr>
						<td width="30%" id="td_left"><?=$m_server_state?></td>
						<td><input type="text" size="20" id="pop3_ip" name="pop3_ip" class="text" value="<?=$cfg_pop3_ip?>"></td>
					</tr>
					<tr>
						<td width="30%" id="td_left"><?=$m_port?></td>
