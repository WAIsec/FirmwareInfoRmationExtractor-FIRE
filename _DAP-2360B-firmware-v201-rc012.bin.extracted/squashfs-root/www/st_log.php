...
function showSysLog()
{
	...
	if (document.layers) return true;
	{
		...
		for (var i=((p-1)*row_num);i < p*row_num;i++)
		{
			...
			priority_message=dataLists[i][1].split("]");
			if (priority_message[0]!="")
			priority_message[0]+="]";
			else
			priority_message[1]="";		
			if(priority_message[1]!=null)
			{
				str+="<tr border=1 borderColor='#ffffff' bgcolor='#dfdfdf'>";
				str+="<td>"+dataLists[i][0]+"</td>";
				str+="<td>"+priority_message[0]+"</td><td>"+genTablelength(priority_message[1],50)+"</td>";		
				str+="</tr>";
			}
		}
		}
